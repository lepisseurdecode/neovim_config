		...
		config = function()
			...
			telescope.setup {
				defaults = {
					...
					mappings = {
						i = {
							['<cr>'] = function(bufnr) set_actions.edit(bufnr, 'tab drop') end,
							['<c-s>'] = function(bufnr) set_actions.edit(bufnr, 'vsplit') end,
						},
					},
				},
			}
			...
			vim.api.nvim_set_keymap('n', '<leader>ftq', '', { noremap = true, callback = builtin.treesitter })
		end,
	},
}
